...
//SJF(Shortest-Job-First) Scheduling 기법
...
void SJF(P *p, int len) {
    ...
    SJF_Run(p, len); // SJF를 실행시키는 함수입니다. 길이가 길어서 따로 뺐습니다.
...
void SJF_Run(P *p, int len) {
    ...
    for(int i = 1; i < len; i ++) {
        ...
        for(int j = 1; j < len; j ++) {
            //최소 작업시간 조건에 맞는지 확인해봅니다.
            if((p[j].complete==FALSE) && (p[j].arrivalTime <= curTime) && (p[j].burstTime < p[min].burstTime)) {
                min = j;
                //최소작업 idx를 갱시해줍니다.
            }
        }
